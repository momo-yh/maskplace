...
def prim_real(vertexs_tmp, node_pos, net_info, ratio, node_info, port_info):
    """
    Calculate the minimum spanning tree cost using Prim's algorithm.

    Args:
        vertexs_tmp (list): List of vertices.
        node_pos (dict): Dictionary containing node positions.
        net_info (dict): Dictionary containing net information.
        ratio (float): Ratio for scaling node positions.
        node_info (dict): Dictionary containing node information.
        port_info (dict): Dictionary containing port information.
